...
锦上添花的技术，在规模不大的工程中运用ServiceWorker或许获得的收益远小于面对的风险。

这个东西，也不是说有益处就一定要上，使用与否要根据业务场景、工程健壮性、测试充分度多种原因作出决定。如无必要，勿增实体。

究其原因是他的缓存是在用户本地，当用户本地遇到因为ServiceWorker导致的问题时，你自己可能连复现都没办法复现，使用时要特别谨慎小心，最好留一个能快速无感关闭用户本地ServiceWorker的后门。
